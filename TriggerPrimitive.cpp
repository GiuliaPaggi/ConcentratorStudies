...
vector<int> TriggerPrimitive::MakeCluster(TriggerPrimitive listOfPrimitives[], int size,
  ...
  for (int i = 0; i < size; ++i) {
    ...
    // double DeltaPhi = 0;
    // abs(element.phi-phi) < TMath::Pi() ? DeltaPhi = abs(element.phi-phi) :
    // DeltaPhi = abs( 2*TMath::Pi() - abs(element.phi-phi));
...
// vector<int> TriggerPrimitive::SelectRIGHT_BX (TriggerPrimitive
// listOfPrimitives[], vector<int> clusterIndex){
//    vector <int> RIGHT_BXCluster;        // = {index}
//    for (auto element : clusterIndex) {
//       if (380 == listOfPrimitives[element].BX)
//       RIGHT_BXCluster.push_back(element);
//    }
//    return RIGHT_BXCluster;
// };
