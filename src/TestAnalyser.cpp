...
void TestAnalyser::DefinePlot(){
  ...
  for (auto CLtype : tags){
    ...
    m_plots[Form("%s_BX_Best", CLtype.c_str())] = new TH1I(Form("%s_BX_Best", CLtype.c_str()), Form("%s_BX_Best; BX; Entries", CLtype.c_str()), 24, BX_MIN, BX_MAX);
    ...
    for (const auto st : STATIONS) {
      ...
      m_effs[Form("%s_ClusterEffprofile_st%d", CLtype.c_str(), st)] = new TEfficiency(Form("%s_ClusterEffprofile_st%d", CLtype.c_str(), st), Form("%s_ClusterEffprofile_st%d; Sector; Efficiency ", CLtype.c_str(), st), 13, -.5, 12.5);
...
void TestAnalyser::ClusterAnalisis(std::vector<Cluster> CLtoAnalize, std::string CLtype,  std::vector<Segment> Segments){
    for (auto const &cluster : CLtoAnalize) {
      ...
      m_plots[Form("%s_BX_Best", CLtype.c_str())]->Fill(cluster.bestTP().BX);
      ...
      if (cluster.muMatched && cluster.bestSeg().nPhiHits >= 4 ) {
        ...
        m_effs[Form("%s_ClusterEffprofile_st%d", CLtype.c_str(), st)]->Fill(efficient, sec);
      }
